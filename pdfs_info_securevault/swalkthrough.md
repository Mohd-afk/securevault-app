# Authentication Flow Fix — Walkthrough

## Problem

Three related bugs allowed the vault to be accessed without the master password:

1. **Google re-login skipped master password** — vault UI opened directly
2. **Empty vault on re-login** — passwords invisible because no decryption key was derived
3. **Login method confusion** — email login failed for accounts created via Google

## Root Cause

In [AppShell.tsx](file:///d:/PYTHON/Password%20Manager/src/app/components/AppShell.tsx), the `onAuthenticated` callback unconditionally set `unlocked=true`, and [AuthScreen.tsx](file:///d:/PYTHON/Password%20Manager/src/app/components/AuthScreen.tsx) called it for returning Google users without any master password verification.

## Changes Made

### [AppShell.tsx](file:///d:/PYTHON/Password%20Manager/src/app/components/AppShell.tsx)
...
### [AuthScreen.tsx](file:///d:/PYTHON/Password%20Manager/src/app/components/AuthScreen.tsx)
...
| Flow | `shouldAutoUnlock` | Result |
|---|---|---|
| Email login (master password entered) | `true` | Vault opens directly ✅ |
| New Google user (master password just created) | `true` | Vault opens directly ✅ |
| **Returning Google user** | **`false`** | **→ LockScreen → master password required** ✅ |
| Magic link setup (master password just created) | `true` | Vault opens directly ✅ |

## Verification

- **Build:** `vite build` passed — 2191 modules transformed, exit code 0
- **Manual testing** of all four flows needed by user in live Firebase environment

> [!IMPORTANT]
> **Bug 3 (email login after Google signup)** needs manual testing. [finalizeMasterPasswordSetup](file:///d:/PYTHON/Password%20Manager/src/app/auth.ts#63-71) calls `updatePassword` which should link the email/password provider to the Google account. If email login still fails after this fix, a follow-up provider-linking change will be needed.
