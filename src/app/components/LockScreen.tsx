      ...
      if (!cancelled) {
        ...
        if (has) {
          ...
          try {
            ...
            log.warn('LockScreen: Failed to check biometric settings', err);
          }

          if (bioAttemptedAndSucceeded) return;
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-4">
          ...
          {!isSetup && showBiometricButton && (
            <button
              ...
              disabled={loading}
              className="w-full bg-[#16213e] hover:bg-[#1a2942] border border-emerald-500/30 text-emerald-400 py-3 rounded-xl disabled:opacity-50 transition-all active:scale-[0.98] flex items-center justify-center gap-2 mt-4"
            >
