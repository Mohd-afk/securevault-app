...
function startRealtimeSync(uid: string | null, password: string): void {
  ...
  _unsubscribeVault = subscribeToVault(uid, async (data) => {
    if (!data || (!isVaultUnlocked())) {
      log.debug('Sync callback: no data or vault is locked, ignoring');
      return;
    }
...
export async function migrateLocalToCloud(): Promise<void> {
  ...
  if (!uid || !isVaultUnlocked()) {
    log.warn('migrateLocalToCloud: no uid or vault locked');
    return;
  }
...
export async function disableBiometricUnlock(): Promise<void> {
  if (Capacitor.getPlatform() === 'android') {
    try {
      ...
      log.warn('disableBiometric failed natively, but proceeding to disable in settings and wiping state', e);
    }
  }
  const settings = await getSettings();
  await saveSettings({ ...settings, biometricEnabled: false });
